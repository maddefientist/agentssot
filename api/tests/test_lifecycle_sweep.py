...
def test_sweep_is_idempotent():
    ...
    with SessionLocal() as s:
        s1 = run_sweep(s, namespace="default", dry_run=True)
        s2 = run_sweep(s, namespace="default", dry_run=True)
...
def test_sweep_decays_low_use_items():
    ...
    with SessionLocal() as s:
        before = s.query(KnowledgeItem).filter(
            ...
            KnowledgeItem.namespace == "default",
        ).count()
        result = run_sweep(s, namespace="default", dry_run=False)
